...
_NORETURN void drDcihLoop( void )
{
    ...
    drDbgPrintLnf("[Driver DrAsyncExample] drDcihLoop(): DCI handler thread is running");
    ...
    for(;;)
    {
        ...
        {
            drDbgPrintLnf("[Driver DrAsyncExample] drDcihLoop(): drApiIpcSigWait failed");
            continue;
        }
        ...
        {
            drDbgPrintLnf("[Driver DrAsyncExample] drDcihLoop(): drApiNotify failed");
        }
    }
}
...
void drDcihInit(
    ...
    {
        drDbgPrintLnf("[Driver DrAsyncExample] drDcihInit(): Updating notification thread failed");
    }
    ...
    {
            drDbgPrintLnf("[Driver DrAsyncExample] drDcihInit(): drApiStartThread failed");
    }
}
