...
_NORETURN void drExchLoop(
    ...
    drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): Exception handler thread is running");
    ...
    for (;;)
    {
                 ...
                 &mr2) )
        {
            ...
            drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): drApiIpcWaitForMessage failed");
            continue;
        }
        ...
        switch(mr1)
        {
            ...
            case TRAP_SEGMENTATION:
                ...
                switch(faultedThread)
                {
                    ...
                    //--------------------------------------
                    default:
                        drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): Unknown thread. This should never happen");
                        break;
                }
                ...
                if ((ip != NULL) && (sp != NULL))
                {
                    ...
                    if (DRAPI_OK != drUtilsRestartThread(
                                    ...
                                    sp))
                    {
                        drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): restarting thread failed");
                    }
                }
            ...
            case TRAP_UNDEF_INSTR:
                ...
                {
                    drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): Unable to stop DCI handler thread");
                }
            #endif // DR_FEATURE_DCI_HANDLER
                ...
                {
                    drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): Unable to stop IPC handler thread");
                }

                /* Stop main thread */
                if (DRAPI_OK != drApiStopThread(NILTHREAD))
                {
                    drDbgPrintLnf("[Driver DrTemplate] drExchLoop(): Unable to stop main thread");
                }
